/* 
Elements à sélectionner : 
- id de la turbine
- année de construction / destruction
- facility
- position x y
- rated power (kW)
*/

const data = require('./turbines.json');

var Swisstopo = {
	WGStoCH: function (lat, lng) {
		return [
			this.WGStoCHy(lat, lng),
			this.WGStoCHx(lat, lng)
		]
	},

	// Convert WGS lat/lng (° dec) to CH x
	WGStoCHx: function (lat, lng) {
		
		// Convert decimal degrees to sexagesimal seconds
		lat = this.DECtoSEX(lat);
		lng = this.DECtoSEX(lng);
		
		// Auxiliary values (% Bern)
		var lat_aux = (lat - 169028.66)/10000;
		var lng_aux = (lng - 26782.5)/10000;

		// Process X
		x = 200147.07 +
			308807.95 * lat_aux  +
			3745.25 * Math.pow(lng_aux,2) +
			76.63 * Math.pow(lat_aux,2) -
			194.56 * Math.pow(lng_aux,2) * lat_aux +
			119.79 * Math.pow(lat_aux,3);

		return x;		
	},

	// Convert WGS lat/lng (° dec) to CH y
	WGStoCHy: function (lat, lng) {
		// Convert decimal degrees to sexagesimal seconds
		lat = this.DECtoSEX(lat);
		lng = this.DECtoSEX(lng);

		// Auxiliary values (% Bern)
		var lat_aux = (lat - 169028.66)/10000;
		var lng_aux = (lng - 26782.5)/10000;
		
		// Process Y
		y = 600072.37 +
			211455.93 * lng_aux -
			10938.51 * lng_aux * lat_aux -
			0.36 * lng_aux * Math.pow(lat_aux,2) -
			44.54 * Math.pow(lng_aux,3);

		return y;
	},

	CHtoWGS: function (y, x) {
		return [
			this.CHtoWGSlng(y, x),
			this.CHtoWGSlat(y, x)
		]
	},

	// Convert CH y/x to WGS lat
	CHtoWGSlat: function (y, x) {
		// Converts military to civil and to unit = 1000km
		// Auxiliary values (% Bern)
		var y_aux = (y - 2600000)/1000000;
		var x_aux = (x - 1200000)/1000000;

		// Process lat
		var lat = 16.9023892 +
			3.238272 * x_aux -
			0.270978 * Math.pow(y_aux, 2) -
			0.002528 * Math.pow(x_aux, 2) -
			0.0447	 * Math.pow(y_aux, 2) * x_aux -
			0.0140	 * Math.pow(x_aux, 3);

		// Unit 10000" to 1 " and converts seconds to degrees (dec)
		lat = lat * 100 / 36;

		return lat;
	},

	// Convert CH y/x to WGS lng
	CHtoWGSlng: function (y, x) {
		// Converts military to civil and	to unit = 1000km
		// Auxiliary values (% Bern)
		var y_aux = (y - 2600000)/1000000; //modification du code original
		var x_aux = (x - 1200000)/1000000;

		// Process lng
		var lng = 2.6779094 +
			4.728982 * y_aux +
			0.791484 * y_aux * x_aux +
			0.1306	 * y_aux * Math.pow(x_aux, 2) -
			0.0436	 * Math.pow(y_aux, 3);

		// Unit 10000" to 1 " and converts seconds to degrees (dec)
		lng = lng * 100 / 36;

		return lng;
	},

	// Convert angle in decimal degrees to sexagesimal seconds
	DECtoSEX: function (angle) {

		// Extract DMS
		var deg = parseInt(angle);
		var min = parseInt((angle-deg)*60);
		var sec = (((angle-deg)*60)-min)*60;	 

		// Result sexagesimal seconds
		return sec + min*60.0 + deg*3600.0;
		
	}
}


years = [];

data.forEach(elt => {
    if(!years.includes(elt.yearOfConstruction)) { years.push(elt.yearOfConstruction)};
});

years.sort();

result = [];

years.forEach(year => {
	const turbines = data.filter(d => d.yearOfConstruction <= year).map(d => d);

	let prepared_turbines = [];

	turbines.forEach(elt => {
		prepared_turbines.push({
			id: elt.xtf_id,
			yearOfConstruction: elt.yearOfConstruction,
			yearOfDismantling: elt.yearOfDismantling,
			longitude: Swisstopo.CHtoWGSlng(elt.x, elt.y),
			latitude: Swisstopo.CHtoWGSlat(elt.x, elt.y),
			ratedPower: elt.ratedPower,
			manufacturer: elt.manufacturer
		  });
	});

	result.push({
		year: year,
		turbines: prepared_turbines
	});
});

console.log(JSON.stringify(result));

/* ### V1 ###
let result = [];

data.forEach(elt => {
  result.push({
    id: elt.xtf_id,
    buildingYear: elt.yearOfConstruction,
    dismantlingYear: elt.yearOfDismantling,
    longitude: Swisstopo.CHtoWGSlng(elt.x, elt.y),
    latitude: Swisstopo.CHtoWGSlat(elt.x, elt.y),
    ratedPower: elt.ratedPower,
    manufacturer: elt.manufacturer
  });
});
*/


/*
let model = [
    {
        year: 2003,
		// !! Contient aussi les éoliennes des années précédentes !!
        turbines: [
            {xtf_id: '...', yearOfConstruction: '...', yearOfDismantling: '...', manufacturer: '...', ratedPower: '...', x: '...', y: '...',},
            {xtf_id: '...', yearOfConstruction: '...', yearOfDismantling: '...', manufacturer: '...', ratedPower: '...', x: '...', y: '...',},
            {...},
        ]
    },
    {
        year: 2004,
        turbines: [
            {xtf_id: '...', yearOfConstruction: '...', yearOfDismantling: '...', manufacturer: '...', ratedPower: '...', x: '...', y: '...',},
			{xtf_id: '...', yearOfConstruction: '...', yearOfDismantling: '...', manufacturer: '...', ratedPower: '...', x: '...', y: '...',},
            {...},
        ]
    }
];
*/